...
model.add(Flatten())
model.add(Dense(512,activation='relu'))  #FC2 10224
model.add(Dropout(rate=0.25))
...
#显示预测结果
plot_images_labels_prediction(x_img_test,y_label_test,prediction,0,9)
...
try:
    model.load_weights("cifarCnnModel-1.h5")
    print("载入模型成功，继续训练模型")
except:
...
train_histroy=model.fit(x_img_train_normalize,y_label_train_Onehot,
                        validation_split=0.2,epochs=2,batch_size=128,verbose=1)

import os
#os.mkdir("SaveModel")
#model.save_weights("SaveModel/cifar10CnnModel-1.h5")
